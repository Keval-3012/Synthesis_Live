...
namespace Repository
{
    public class SynthesisApiRepository : ISynthesisApiRepository
    {
        ...
        public async Task<string> PosttDataEmployeeLogin(string UserName, string Password)
        {
            string Response = "", accessToken = "";
            try
            {
        ...
        public async Task<string> CompleteTraining(CompleteyTraining data)
        {
            string Response = "";
            DataTable dt = new DataTable();
            try
            {
                ...
                var client = new HttpClient();
                var request = new HttpRequestMessage(HttpMethod.Post, ConfigurationManager.AppSettings["ApiUrl"].ToString() + "/api/EmployeeTraining/CompleteTraining");
                request.Headers.Add("Authorization", $"Bearer {Convert.ToString(HttpContext.Current.Session["AccessTokan"])}");
                var content = new StringContent(jsonString, null, "application/json");
                request.Content = content;
                var response = await client.SendAsync(request);
                Response = await response.Content.ReadAsStringAsync();
            ...
            {
                logger.Error("SynthesisApiRepository - CompleteTraining - " + DateTime.Now + " - " + ex.Message.ToString());
            }
            return Response;
        }

        public async Task<string> ResetTraining(ResetTraining data)
        {
            string Response = "";
            DataTable dt = new DataTable();
            try
            {
                var jsonString = JsonConvert.SerializeObject(data);
                var client = new HttpClient();
                var request = new HttpRequestMessage(HttpMethod.Post, ConfigurationManager.AppSettings["ApiUrl"].ToString() + "/api/EmployeeTraining/ResetTraining");
                request.Headers.Add("Authorization", $"Bearer {Convert.ToString(HttpContext.Current.Session["AccessTokan"])}");
                var content = new StringContent(jsonString, null, "application/json");
                request.Content = content;
                var response = await client.SendAsync(request);
            ...
            {
                logger.Error("SynthesisApiRepository - ResetTraining - " + DateTime.Now + " - " + ex.Message.ToString());
            }
            return Response;
        }

        public async Task<string> GetNoteTitleFromGPTAsync(string content)
        {
            string apiUrl = ConfigurationManager.AppSettings["APIGptUrl"];
            string accessToken = ConfigurationManager.AppSettings["APIKey"];
            ...
            try
            {
                using (var client = new HttpClient())
                {
                    ...
                    var requestData = new
                    {
                        ...
                        messages = new[]
                        {
                    new { role = "system", content = "## ChatGPT Task - Given the content of a note (text, paragraph, or bullet points), generate a concise and accurate title for the note, no longer than 10 words." },
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        var messageContent = jsonResult.choices[0].message.content.ToString();
                        title = messageContent.Trim().Trim(new char[] { '"', '\'', '\\', '/', '<', '>', });
                        //title = messageContent.Replace("<note-title>", string.Empty).Replace("</note-title>", string.Empty).Trim();
                    }
                    else
                    {
                        throw new Exception($"Failed to call OpenAI API. Status code: {response.StatusCode}");
                    }
                }
            }
            catch (Exception ex)
            {
                logger.Error("Failed to get title from GPT: " + ex.Message);
            }
