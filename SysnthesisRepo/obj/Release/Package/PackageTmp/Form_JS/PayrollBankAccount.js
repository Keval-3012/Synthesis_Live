...
function Updatedata(ID) {
    ...
    var SortingNo = document.getElementById("ESortingNo_" + ID).value;
    var cancel = "#" + "cancel_" + ID;
    var update = "#" + "update_" + ID;
    ...
    if (SortingNo == "" || SortingNo == "0") {
        $("#ESortingNoReq" + ID).show();
        return false;
    }
    if (CheckDuplicate(Temp, SortingNo) == true) {
        return false;
    }

    $.ajax({
        url: ROOTURL + '/PayrollAccount/UpdatePayrollAccount',
        data: { ID: ID, QBAccountid: QBval, Description: DescVal, ValueIn: ValueInVal, IsActive: IsActiveVal, Sortingno: SortingNo },
        type: 'POST',
        cache: false,
        dataType: 'json',
        ...
        success: function (response) {
            ...
            else if (response === "Exists") { MyCustomToster('Payroll Account Name Is Already Exists.'); }
            Loader(0);
        }
    });
}
