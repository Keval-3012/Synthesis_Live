...
//for invoiceautomationedit updated by Dani on 12-11-2025 to handle the total amount to first department & rest of dept set the amount to be 0.00.
    ...
    if (VendorId != "") {
        $.ajax({
            ...
            success: function (states) {
                ...
                var rowCount = $(".tbladdoncls .trrowcls").length;
                for (var i = rowCount; i > 1; i--) {
                    $("#trrow_" + i).remove();
                }

                // Set first department with total amount
                if (states.length >= 1) {
                    $("#Drp_1").val(states[0]);
                    // Keep the existing amount in first row (total amount)
                    // Don't modify Amt_1 as it already has the total amount
                }

                // Add remaining departments with 0.00 amount
                lastVal = 1;
                if (states.length > 1) {
                    for (var j = 1; j < states.length; j++) {
                        ...
                        $("#Drp_" + nextVal).val(states[j]);

                        // Set amount to 0.00 for remaining departments
                        $("#Amt_" + nextVal).val("0.00");
                ...
                // Recalculate total after populating departments
                AddAmt();
                Calculatediscount();
            }
        });
