...
public class Export
{
    ...
    public void ExportDetails(DataTable DetailsTable, ExportFormat FormatType, string FileName,string sFilterString)
    {
        try
        {
            ...
            if (appType == "Web")
                Export_with_XSLT_Web(dsExport, sHeaders, sFileds, FormatType, FileName, sFilterString, "Invoice Reports");
    ...
    public void ExportDetails(DataTable DetailsTable, int[] ColumnList, ExportFormat FormatType, string FileName,string sFilterString)
    {
        try
        {
            if (DetailsTable.Rows.Count == 0)
    ...
    public void ExportDetails(DataTable DetailsTable, int[] ColumnList, string[] Headers, ExportFormat FormatType,
        string FileName,string sFilterString,string Header)
    {
        //try
        //{
        if (DetailsTable.Rows.Count == 0)
        ...
        for (int i = 0; i < ColumnList.Length; i++)
        {
            if ((ColumnList[i] < 0) || (ColumnList.Length > dtExport.Columns.Count))
                throw new Exception("ExportColumn Number should not exceed Total Columns Range");
        ...
        if (appType == "Web")
            Export_with_XSLT_Web(dsExport, Headers, sFileds, FormatType, FileName, sFilterString,Header);
    ...
    private void Export_with_XSLT_Web(DataSet dsExport, string[] sHeaders, string[] sFileds, ExportFormat FormatType, string FileName, string sFilterString,string Header)
    {
        try
        {
            // Appending Headers
            ...
            CreateStylesheet(writer, sHeaders, sFileds, FormatType, sFilterString,Header);
            writer.Flush();
    ...
    private void CreateStylesheet(XmlTextWriter writer, string[] sHeaders, string[] sFileds, ExportFormat FormatType,string sFilterString,string Header)
    {
        try
        {
            // xsl:stylesheet
            ...
            writer.WriteAttributeString("select", "'"  +  Header +"'");
            ...
            if (sFilterString != "")
            {
                writer.WriteString("\"");
                writer.WriteStartElement("xsl:value-of");
                writer.WriteAttributeString("select", "'Filter By: " + sFilterString +"'");
                writer.WriteEndElement(); // xsl:value-of
                writer.WriteString("\"");
                writer.WriteString((FormatType == ExportFormat.CSV) ? "," : "	");
                writer.WriteString("\r\n");
